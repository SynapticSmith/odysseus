...
This section outlines the high-level boundaries and responsibilities within the repository. For exhaustive file-by-file details, see the sections below.

- **`.github/`**: CI/CD workflows, issue templates, and PR validators.
- **`config/`**: System configuration files (e.g., SearXNG settings).
- **`core/`**: Central platform primitives—authentication, atomic I/O, database sessions, and fundamental abstract models.
- **`docs/`**: Project documentation, including this architecture report.
- **`docker/`**: Containerization profiles supporting both NVIDIA and AMD GPUs natively.
- **`integrations/`**: Plugins bridging the core system to external services.
- **`mcp_servers/`**: Embedded Model Context Protocol instances for localized data querying.
- **`routes/`**: FastAPI REST and SSE endpoints bridging the frontend and backend core.
- **`scripts/`**: Offline administration, CI validation, and database maintenance scripts.
- **`services/`**: Long-running background workers including TTS, STT, and task schedulers.
- **`src/`**: Primary application business logic, AI orchestration loops, and integration managers.
- **`static/`**: Vanilla JavaScript frontend application, heavily modularized.
- **`tests/`**: Comprehensive test suites split into isolated domains.

### Architecture Overview

Odysseus consists of a tightly coupled Python backend and a Vanilla JS frontend, functioning as a single localized application.

**Entry Points & Data Flows:**
1. **Frontend (`static/`)**: A modular SPA architecture utilizing native ES6 imports without bundlers. Websockets and Server-Sent Events (SSE) power real-time interactions.
2. **API Layer (`routes/`)**: FastAPI orchestrates requests. HTTP REST routes handle standard CRUD, while streaming endpoints handle long-running LLM inferences.
3. **Core Orchestration (`src/agent_loop.py`)**: Incoming complex prompts are dynamically analyzed and routed to various embedded agents. The Agent Loop manages recursive tool-calling until a halting condition is met.
4. **Data Persistence (`core/database.py` & ChromaDB)**: Structured data resides in SQLite, while vector embeddings flow to ChromaDB for semantic retrieval.
5. **Background Workers (`services/`)**: The Event Bus triggers localized background task execution (e.g., transcription or web scraping) autonomously, decoupled from active user requests.

**Ownership Boundaries:**
- The Backend strictly owns data persistence, access control, AI orchestration, and system interfaces.
- The Frontend strictly manages display state, rendering logic, and local browser-based caching.
...
- **[`routes/_validators.py`](../routes/_validators.py)**: Core module recently added for modularity.
- **[`routes/api_token_routes.py`](../routes/api_token_routes.py)**: Core module recently added for modularity.
- **[`routes/chat_helpers.py`](../routes/chat_helpers.py)**: Core module recently added for modularity.
- **[`routes/chat_routes.py`](../routes/chat_routes.py)**: Core module recently added for modularity.
- **[`routes/cookbook_helpers.py`](../routes/cookbook_helpers.py)**: Core module recently added for modularity.
- **[`routes/cookbook_output.py`](../routes/cookbook_output.py)**: Core module recently added for modularity.
- **[`routes/document_helpers.py`](../routes/document_helpers.py)**: Core module recently added for modularity.
- **[`routes/document_routes.py`](../routes/document_routes.py)**: Core module recently added for modularity.
- **[`routes/email_helpers.py`](../routes/email_helpers.py)**: Core module recently added for modularity.
- **[`routes/email_routes.py`](../routes/email_routes.py)**: Core module recently added for modularity.
- **[`routes/memory_routes.py`](../routes/memory_routes.py)**: Core module recently added for modularity.
- **[`routes/note_routes.py`](../routes/note_routes.py)**: Core module recently added for modularity.
- **[`routes/preset_routes.py`](../routes/preset_routes.py)**: Core module recently added for modularity.
- **[`routes/skills_routes.py`](../routes/skills_routes.py)**: Core module recently added for modularity.
- **[`routes/webhook_routes.py`](../routes/webhook_routes.py)**: Core module recently added for modularity.
- **[`routes/workspace_routes.py`](../routes/workspace_routes.py)**: Core module recently added for modularity.
...
- **[`src/optional_deps.py`](../src/optional_deps.py)**: Core module recently added for modularity.
- **[`src/pdf_forms.py`](../src/pdf_forms.py)**: Core module recently added for modularity.
